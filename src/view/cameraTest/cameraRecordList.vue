<template>
  <div>
    <div>
      ...
      <el-button type="primary" size="small" @click="searchList" id="searchButton" style="margin-left:20px"
                 icon="el-icon-search">搜索
      </el-button>
      <!-- addForm -->
      <el-button type="primary" @click="state.addShow=true" size="small" style="margin-left:10px" icon="el-icon-plus">
        增加
      </el-button>
      ...
      <el-dialog title="编辑进出记录" :visible.sync="state.editShow" width="340px">
        <edit ref="editForm" v-bind:editForm="editForm" v-bind:userOptions="userOptions" @message="message"
              @refresh="getList"></edit>
      </el-dialog>
    ...
    <el-table :data="list" style="width: 100%" :height="this.$store.getters.getTableHeight" ref="table"
      ...
      <el-table-column label="抓拍照片">
        <template slot-scope="scope">
          <i class="el-icon-search" v-show="scope.row.captureName!=null" style="color:#2b57ff"
             @click="showCapture(scope.row.captureName)"> </i>
        </template>
      </el-table-column>
